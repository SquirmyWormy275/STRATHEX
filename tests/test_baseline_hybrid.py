...
def test_fit_hierarchical_regression(sample_results_df, sample_wood_df):
    ...
    model = fit_hierarchical_regression(cleaned_df, sample_wood_df, hardness_index, half_lives)
...
def test_group_wise_bias_correction():
    ...
    predictions_dict = {"Alice": 25.0, "Bob": 40.0, "Charlie": 50.0, "David": 60.0}
...
def test_apply_convergence_adjustment():
    ...
    adjusted = apply_convergence_adjustment(
        predictions_dict, diameter=300, event_code="SB", target_spread=2.0, preserve_ranking=True
    )
...
def test_calibrate_predictions_for_handicapping():
    ...
    calibrated, metadata = calibrate_predictions_for_handicapping(predictions_dict, 300, "SB", results_df)
...
def test_predict_baseline_v2_hybrid_tournament_weighting():
    ...
    if meta_with:
        assert meta_with.get("tournament_weighted", False)
...
def test_prediction_consistency():
    ...
    pred1, conf1, exp1, meta1 = predict_baseline_v2_hybrid("Arden Cogar Jr", "S01", 300, 5, "SB", results_df, wood_df)

    pred2, conf2, exp2, meta2 = predict_baseline_v2_hybrid("Arden Cogar Jr", "S01", 300, 5, "SB", results_df, wood_df)
